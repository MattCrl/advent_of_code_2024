...
def solve_part_one(inputs)
  ...
  inputs.each do |report|
    ...
    valid_count += 1 if diff_acceptable?(report, 1, 3) && is_sorted?(report)
  end
  valid_count
end
...
def create_sub_arrays(initial_report)
  ...
  initial_report.each_index.map do |index|
  ...
  inputs.each do |report|
    ...
    if diff_acceptable?(report, 1, 3) && is_sorted?(report)
      ...
      sub_reports.each do |sub_report|
        sub_valid = true if diff_acceptable?(sub_report, 1, 3) && is_sorted?(sub_report)
      end
